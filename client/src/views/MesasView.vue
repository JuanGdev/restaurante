  ...
  <v-container>
    ...
    >
      ...
      <template v-slot:[`item.actions`]="{ item }">
        <v-btn small dark rounded color="teal_l" @click="definirMesa(item)">
          Ocupar mesa
        </v-btn>
      </template>
    ...
    <v-dialog v-model="mesa_dialog" max-width="500px">
      <v-card>
        <v-card-title> Asignar mesero </v-card-title>
        <v-card-text>
          <v-row>
            <v-col cols="12">
              <v-select
  ...
  methods: {
    ...
    async guardar() {
      ...
      await this.axios.put(
        "/mesas/estado_ocupada/" + this.nueva_orden.ord_mesa_id.toString()
      );
